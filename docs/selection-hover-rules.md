...
1. For hover resolution on pictures:
   ...
   - **Non-texture pictures** (no `texture && mask`) are visible only on a pixel-valid area:
     - `isWorldPointInPictureVisiblePixel(...)` must be true
     - and clipping rules must allow the point.
   - Non-texture pictures do not fall back to rectangular bounds in any visibility mode.
...
1. Non-texture pictures keep the existing region clipping behavior.
